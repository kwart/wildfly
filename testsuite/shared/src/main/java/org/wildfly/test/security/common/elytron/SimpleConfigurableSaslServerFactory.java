...
public class SimpleConfigurableSaslServerFactory extends AbstractConfigurableElement implements SaslServerFactory {
    ...
    public void create(ModelControllerClient client, CLIWrapper cli) throws Exception {
        ...
        if (!filters.isEmpty()) {
            ...
            for (SaslFilter saslFilter : filters) {
                ...
                setAttribute(saslFilterNode, "predefined-filter", saslFilter.getPredefinedFilter());
                setAttribute(saslFilterNode, "pattern-filter", saslFilter.getPatternFilter());
                setAttribute(saslFilterNode, "enabling", saslFilter.isEnabling());
        ...
        setAttribute(op, "protocol", protocol);
        op.get("sasl-server-factory").set(saslServerFactory);
        setAttribute(op, "server-name", serverName);
    ...
    private void setAttribute(ModelNode node, String attribute, Boolean value) {
        if (value != null) {
            node.get(attribute).set(value);
        }
    }
